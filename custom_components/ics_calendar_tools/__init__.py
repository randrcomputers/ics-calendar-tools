...
async def _run_reload_script_like_ui(hass: HomeAssistant) -> None:
    """
    EXACTLY like your Dev Tools -> Actions test:
      action: script.turn_on
      target:
        entity_id: script.reload_local_calendars
    """
    # Make it super obvious in logs when we try
    _LOGGER.warning("ICS_CALENDAR_TOOLS: triggering %s via script.turn_on", RELOAD_SCRIPT_ENTITY_ID)

    await hass.services.async_call(
        "script",
        "turn_on",
        {},
        target={"entity_id": RELOAD_SCRIPT_ENTITY_ID},
        blocking=True,
    )
...
async def _force_refresh_after_edit(hass: HomeAssistant, cal_ent: str, ics_path: str, before_mtime: float | None) -> None:
    ...
    # Run the reload script (same behavior as your manual action)
    try:
        await _run_reload_script_like_ui(hass)
    except Exception as e:
        _LOGGER.error("ICS_CALENDAR_TOOLS: script reload failed: %s", e)
        return

    # Local Calendar can still be sluggish — run it once more after a short pause
